...
namespace Asaki.Editor.Utilities.Tools
{
    public class AsakiQuickLayoutWindow : EditorWindow
    {
        ...
        {
            var window = GetWindow<AsakiQuickLayoutWindow>("Quick Layout");
            window.minSize = new Vector2(320, 500);
            window.Show();
        }

        [MenuItem("GameObject/Asaki/Quick Layout", false, 0)] // 右键菜单
        public static void ShowWindowFromHierarchy() => ShowWindow();
        ...
        private void ApplyLayoutPreview() => ApplyLayoutInternal(recordUndo: false);
        ...
        private float GetSpacingFor(Axis axis) => axis switch
        {
            Axis.X => _spacing.x,
            Axis.Y => _spacing.y,
            Axis.Z => _spacing.z,
        ...
        {
            if (_anchorMode != AnchorMode.Pivot || !t.TryGetComponent<RectTransform>(out var rect))
                return 0;
        ...
        private bool HasValidSelection() => Selection.transforms.Length >= 2;
