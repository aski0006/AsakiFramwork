...
namespace Asaki.Editor.Utilities.Tools.AssetsExplore
{
    public class AssetScanner
    {
            ...
            try
            {
                ...
                var tasks = allFiles.Select(async filePath => 
                {
                    ...
                    try
                    {
                        ...
                        // 增量检查：利用文件修改时间
                        if (incremental && _cachedAssets.TryGetValue(relativePath, out var cachedInfo))
                        {
                            // 误差在 1 秒内认为未修改
                        ...
                        var info = await CreateAssetInfoNative(filePath, relativePath, currentModTime);
                        if (info != null)
                        {
        ...
        private List<string> CollectAllFiles(CancellationToken token)
        {
            ...
            foreach (var rootPath in _scanPaths)
            {
                ...
                try 
                {
                    ...
                    foreach (var file in files)
                    {
                        ...
                        {
                            if (normalizedPath.Contains(ex)) { excluded = true; break; }
                        }
        ...
        #region Cache IO
        [Serializable]
        private class CacheWrapper { public AssetInfo[] items; public long timestamp; }
        ...
        private void LoadCache()
        {
            ...
            try
            {
                ...
                var data = JsonUtility.FromJson<CacheWrapper>(json);
                if (data?.items != null)
                {
        ...
        private void SaveCache()
        {
            try
            {
                var wrapper = new CacheWrapper { items = _cachedAssets.Values.ToArray(), timestamp = DateTime.UtcNow.Ticks };
