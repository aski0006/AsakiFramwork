...
namespace Asaki.Core.Graphs
{
		...
		where TGraph : AsakiGraphAsset
	{
		...
		protected virtual void ExecuteNode(AsakiNodeBase node)
		{
			...
			// --- 针对 SetVariableNode 的特殊优化 ---
			if (node is AsakiSetVariableNode setVarNode)
			{
				if (_context.Blackboard == null) return;

				// 1. 获取要写入的值 (此时是 object)
				object valueToWrite = GetInputValue<object>(setVarNode, "Value");

				// 2. 写入黑板 (传入节点中存储的 VariableTypeName)
				// 这里的 setVarNode.VariableTypeName 就是我们在编辑器里保存的类型全名
		...
		/// <param name="typeName">变量类型全名。</param>
		/// <remarks>
			...
			switch (value)
			{
				...
				default:
					if (!string.IsNullOrEmpty(typeName) &&
					    AsakiTypeBridge.TrySetValue(_context.Blackboard, key, typeName, value))
					{
						return;
					}
					try
