...
namespace Asaki.Core.Logging
{
    public class AsakiLogAggregator
    {
        ...
        private struct LogSignature : IEquatable<LogSignature>
        {
            ...
            public bool Equals(LogSignature other) => _hash == other._hash;
        }
        ...
        public void Sync(int batchLimit = 1000)
        {
            int count = 0;
            // 批量从并发队列取出
            while (count < batchLimit && _inputQueue.TryDequeue(out var packet))
            {
                count++;
        ...
        {
            var sig = new LogSignature(p.File, p.Line, p.Message);

            // 锁定显示列表 (为了 Snapshot 安全)
            lock (_snapshotLock)
            {
                if (_signatureMap.TryGetValue(sig, out var model))
                {
                    // === Inc ===
                    model.Count++;
